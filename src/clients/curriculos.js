document.addEventListener('DOMContentLoaded', function () {
  ...
  // Adicionar um ouvinte de evento ao formulário para adicionar currículos
...
function adicionarCurriculo() {
  ...
  const linkParaCurriculo = document.getElementById('linkCurriculo').value;
  const telefone = document.getElementById('telefoneCurriculo').value;

  fetch('http://localhost:3000/curriculos', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
          nome: nome,
          endereco: endereco,
          linkParaCurriculo: linkParaCurriculo,
          telefone: telefone,
      }),
  })
